...
echo "Running tracking combining for run ${run}, seg ${seg}"
echo "---------------------------------------------"
...
echo "INFO: tracking combining input segment ${seg} maps to output segments ${startseg}..${endseg}."
echo "---------------------------------------------"
...
root_line="Fun4All_Combining.C(${nevents},\"${outdir}\",\"${stump}\",${neventsper},${startseg},\"${dbtag}\",\"infile.list\",\"${histdir}\")"
...
echo "INFO: tracking combining completed for input segment ${seg}; output segments ${startseg}..${endseg}."
